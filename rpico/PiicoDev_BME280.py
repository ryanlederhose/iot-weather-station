...
class PiicoDev_BME280:
    ...
    def __init__(self, t_mode=2, p_mode=5, h_mode=1, iir=1, address=0x77, i2c=None):
    ...
    def _write8(self, reg, dat):
        if reg is None:
            self.i2c.writeto(self.addr,  bytes([dat]))
        else:
            self.i2c.writeto(self.addr, bytes([reg]) + bytes([dat]))
    ...
    def read_raw_data(self):
        ...
        while (self._read16(0xF3) & 0x08):
            sleep_ms(1)
        raw_p = ((self._read8(0xF7) << 16) | (
            self._read8(0xF8) << 8) | self._read8(0xF9)) >> 4
        raw_t = ((self._read8(0xFA) << 16) | (
            self._read8(0xFB) << 8) | self._read8(0xFC)) >> 4
